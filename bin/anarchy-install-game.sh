...
if ! [ -e "$EXECUTABLE" ]; then
    EXECUTABLE=$(zenity --file-selection --title="Select the installer" --filename="$(pwd)" &> /dev/null)
    echo "You selected: $EXECUTABLE"
fi

echo "Preparing to install non steam game $EXECUTABLE"
GAME_NAME="${GAME_NAME:-$(zenity --entry --title="Game Name" --entry-text "$(basename "$(dirname "$EXECUTABLE")")"  &> /dev/null)}"
GAME_NAME_SANITIZED=$(echo "$GAME_NAME" | tr '[:upper:]' '[:lower:]' | sed -E 's/[[:space:]]+/-/g; s/-+/-/g')
DESTINATION="$(zenity --entry --title="Destination" --entry-text "$(dirname $ORIGINAL_PATH)/$GAME_NAME_SANITIZED"  &> /dev/null)"
mkdir -p "$DESTINATION"
...
./anarchy-run.sh "$EXECUTABLE" || zenity --question --title="Confirmation" --text="Could not detect successful installation, do you want to continue with steam integration?"  &> /dev/null

echo "Preparing to pick game executable file"
EXECUTABLE="$(zenity --file-selection --title="Select the game executable" --filename="$GAME_LINK_DIR"  &> /dev/null)"
...
( zenity --info --text="$GAME added! Please restart steam"  &> /dev/null & )
